...
- (instancetype)init:(PHAssetResource *)phAssetResource
			   error:(NSError *__autoreleasing *)error  API_AVAILABLE(ios(9.0)){
    if (self = [super init]) {
...
- (NSData *)read:(long)offset size:(long)size {
	...
	if (@available(iOS 9.0, *)) {
		...
		subData = [self.assetData subdataWithRange:subRange];
	}
...
- (NSData *)fetchDataFromAsset:(PHAssetResource *)videoResource  API_AVAILABLE(ios(9.0)){
