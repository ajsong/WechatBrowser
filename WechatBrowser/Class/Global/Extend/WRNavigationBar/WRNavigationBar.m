...
static char kWRDefaultNavBarShadowImageColorKey;
...
+ (UIColor *)defaultNavBarShadowImageColor {
	UIColor *color = objc_getAssociatedObject(self, &kWRDefaultNavBarShadowImageColorKey);
	return (color != nil) ? color : [UIColor colorWithRed:197.f/255.0 green:197.f/255.0 blue:197.f/255.0 alpha:1.f];
}
+ (void)wr_setDefaultNavBarShadowImageColor:(UIColor *)color {
	objc_setAssociatedObject(self, &kWRDefaultNavBarShadowImageColorKey, color, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (CGFloat)wrPopProgress {
    ...
    CGFloat current = MIN(all, wrPopDisplayCount);
    return current / all;
}
...
- (CGFloat)wrPushProgress {
    ...
    CGFloat current = MIN(all, wrPushDisplayCount);
    return current / all;
}
...
- (void)setNeedsNavigationBarUpdateForShadowImageHidden:(BOOL)hidden {
	...
	UIImageView *shadowImage = [self getShadowImageForView:self.navigationBar];
	if (shadowImage) shadowImage.hidden = hidden;
}
- (void)setNeedsNavigationBarUpdateForShadowImageColor:(UIColor *)shadowImageColor {
	UIImageView *shadowImage = [self getShadowImageForView:self.navigationBar];
	if (shadowImage) {
		UIView *colorView = [[UIView alloc]initWithFrame:shadowImage.bounds];
		colorView.backgroundColor = shadowImageColor;
		colorView.tag = 753829;
		[shadowImage addSubview:colorView];
	}
}
- (UIImageView*)getShadowImageForView:(UIView*)view{
	if ([view isKindOfClass:[UIImageView class]] && view.bounds.size.height<=1.0*SCREEN_SCALE) {
		return (UIImageView*)view;
	}
	for (UIView *subview in view.subviews) {
		UIImageView *imageView = [self getShadowImageForView:subview];
		if (imageView) {
			return imageView;
			break;
		}
...
- (NSArray<UIViewController *> *)wr_popToViewController:(UIViewController *)viewController animated:(BOOL)animated {
	...
	NSArray<UIViewController *> *vcs = nil;
	if (viewController) {
		[CATransaction setAnimationDuration:wrPopDuration];
		[CATransaction begin];
		vcs = [self wr_popToViewController:viewController animated:animated];
...
- (BOOL)navigationBar:(UINavigationBar *)navigationBar shouldPopItem:(UINavigationItem *)item {
	...
	if ([self isKindOfClass:[KKNavigationController class]]) {
		popToVC = [(KKNavigationController*)self popViewControllerForPopItemAnimated:YES];
	} else {
		NSUInteger itemCount = self.navigationBar.items.count;
		NSUInteger n = self.viewControllers.count >= itemCount ? 2 : 1;
		popToVC = self.viewControllers[self.viewControllers.count - n];
	}
...
static char kWRNavBarShadowImageColorKey;
static char kWRNavBarTranslationYKey;
...
// shadowImageColor
- (UIColor *)wr_navBarShadowImageColor {
	UIColor *color = objc_getAssociatedObject(self, &kWRNavBarShadowImageColorKey);
	return (color != nil) ? color : [WRNavigationBar defaultNavBarShadowImageColor];
}
- (void)wr_setNavBarShadowImageColor:(UIColor *)color {
	objc_setAssociatedObject(self, &kWRNavBarShadowImageColorKey, color, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
	[self.navigationController setNeedsNavigationBarUpdateForShadowImageColor:color];
}

// translationY
- (CGFloat)wr_translationY{
	id translationY = objc_getAssociatedObject(self, &kWRNavBarTranslationYKey);
	return (translationY != nil) ? [translationY floatValue] : 0;
}
- (void)wr_setTranslationY:(CGFloat)translationY{
	objc_setAssociatedObject(self, &kWRNavBarTranslationYKey, @(translationY), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)wr_viewWillAppear:(BOOL)animated {
	...
	if (self.tabBarControllerKK &&
		![self.navigationController.viewControllers.firstObject isEqual:self] &&
		self.navigationController.viewControllers.firstObject.hidesBottomBarWhenPushedKK) {
    ...
    if ([self canUpdateNavigationBar] == YES)
    {
        ...
        if (barBgImage != nil) {
            ...
            if ([WRNavigationBar needUpdateNavigationBar:self]) {
				...
				[self.navigationController setNeedsNavigationBarUpdateForShadowImageColor:[self wr_navBarShadowImageColor]];
            }
        }
    }
