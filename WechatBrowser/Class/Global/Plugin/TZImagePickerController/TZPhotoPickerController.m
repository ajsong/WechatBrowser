...
- (void)viewDidLoad {
    [super viewDidLoad];
	[self.navigationController setBackgroundColor:COLORRGB(@"414141") textColor:COLORWHITE];
...
- (void)configBottomToolBar {
    ...
    _previewButton.titleLabel.font = FONT(14);
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        _originalPhotoButton.imageEdgeInsets = UIEdgeInsetsMake(0, [TZCommonTools tz_isRightToLeftLayout] ? 10*SCREEN_SCALE : -10*SCREEN_SCALE, 0, 0);
        [_originalPhotoButton addTarget:self action:@selector(originalPhotoButtonClick) forControlEvents:UIControlEventTouchUpInside];
        _originalPhotoButton.titleLabel.font = _previewButton.titleLabel.font;
        ...
        _originalPhotoLabel.font = _previewButton.titleLabel.font;
        _originalPhotoLabel.textColor = [UIColor blackColor];
        if (_isSelectOriginalPhoto) [self getSelectedPhotoBytes];
    }
    
    _doneButton = [[UIButton alloc]init];
    _doneButton.titleLabel.font = _previewButton.titleLabel.font;
    ...
    _numberLabel.font = _previewButton.titleLabel.font;
...
- (void)viewDidLayoutSubviews {
    ...
    CGFloat toolBarHeight = [TZCommonTools tz_isIPhoneX] ? 50*SCREEN_SCALE + (83 - 49)*SCREEN_SCALE : 50*SCREEN_SCALE;
    ...
    CGFloat previewWidth = [tzImagePickerVc.previewBtnTitleStr boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:FONT(16)} context:nil].size.width + 2*SCREEN_SCALE;
    if (!tzImagePickerVc.allowPreview) {
        previewWidth = 0.0;
    }
    _previewButton.frame = CGRectMake(10*SCREEN_SCALE, 3*SCREEN_SCALE, previewWidth, 44*SCREEN_SCALE);
    _previewButton.tz_width = !tzImagePickerVc.showSelectBtn ? 0 : previewWidth;
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        CGFloat fullImageWidth = [tzImagePickerVc.fullImageBtnTitleStr boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX) options:NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:FONT(13)} context:nil].size.width;
        _originalPhotoButton.frame = CGRectMake(CGRectGetMaxX(_previewButton.frame), 0, fullImageWidth + 56*SCREEN_SCALE, 50*SCREEN_SCALE);
        _originalPhotoLabel.frame = CGRectMake(fullImageWidth + 46*SCREEN_SCALE, 0, 80*SCREEN_SCALE, 50*SCREEN_SCALE);
    }
    [_doneButton sizeToFit];
    _doneButton.frame = CGRectMake(self.view.tz_width - _doneButton.tz_width - 12*SCREEN_SCALE, 0, _doneButton.tz_width, 50*SCREEN_SCALE);
    _numberImageView.frame = CGRectMake(_doneButton.tz_left - 24*SCREEN_SCALE - 5*SCREEN_SCALE, 13*SCREEN_SCALE, 24*SCREEN_SCALE, 24*SCREEN_SCALE);
    _numberLabel.frame = _numberImageView.frame;
    _divideLine.frame = CGRectMake(0, 0, self.view.tz_width, 1*SCREEN_SCALE);
