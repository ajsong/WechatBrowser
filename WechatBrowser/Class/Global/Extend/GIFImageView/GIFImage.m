...
+ (id)imageWithContentsOfFile:(NSString *)path{
    return [self imageWithData:[NSData dataWithContentsOfFile:path] scale:isRetinaFilePath(path) ? 2.0f : 1.0f];
}
...
+ (BOOL)isGif:(NSData *)data{
	...
	CGImageSourceRef imageSource = CGImageSourceCreateWithData((__bridge CFDataRef)(data), NULL);
	BOOL isGif = CGImageSourceContainsAnimatedGif(imageSource);
	CFRelease(imageSource);
	return isGif;
}
...
- (id)initWithCGImageSource:(CGImageSourceRef)imageSource scale:(CGFloat)scale{
    self = [super init];
    if (!imageSource || !self) return nil;
...
- (UIImage*)getFrameWithIndex:(NSUInteger)idx{
    //if ([self.images[idx] isKindOfClass:[NSNull class]]) return nil;
    UIImage *frame = nil;
