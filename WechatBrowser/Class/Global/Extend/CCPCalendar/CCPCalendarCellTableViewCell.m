...
- (UIView *)createDateView:(NSDate *)date {
	...
	bigLabel.textColor = self.manager.normalTextColor;
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        NSString *ym = [NSString stringWithFormat:@"%ld%02ld%02ld",(long)[date getYear],(long)[date getMonth],(long)i];
        btn.tag = [ym integerValue];
        btn.date = [date changToDay:i - week + 1];
        btn.manager = self.manager;
        btn.enabled = NO;
        if (i >= week && i < (count + week_last - 6)) {
            NSString * titleNum = [NSString stringWithFormat:@"%ld",(long)(i - week + 1)];
            btn.enabled = YES;
            [self manageClick];
            [btn ccpDispaly];
...
- (void)manageClick {
    ...
    self.manager.click = ^(UIButton *abtn) {
        if (ws.manager.selectType == selectTypeSingle) {
        ...
        else if (ws.manager.selectType == selectTypeMultiple) {
		...
		else if (ws.manager.selectType == selectTypeSingleMultiple) {
			BOOL nonBtn = YES;
			for (UIButton *lastBtn in ws.manager.selectBtns) {
				if (abtn == lastBtn) {
					nonBtn = NO;
					abtn.selected = NO;
					[ws.manager.selectBtns removeObject:abtn];
					break;
				}
			}
			if (nonBtn) [ws.manager.selectBtns addObject:abtn];
        }
