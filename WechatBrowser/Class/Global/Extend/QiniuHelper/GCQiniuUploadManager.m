...
- (void)createToken {
    ...
    [authInfo setObject:[NSNumber numberWithLong:[[NSDate date] timeIntervalSince1970] + self.liveTime * 24 * 3600] forKey:@"deadline"];
            ...
            complete:^(QNResponseInfo *info, NSString *key, NSDictionary *resp) {
                if (completion) {
                    if (info.error) {
                        ...
                        NSString *link = [NSString stringWithFormat:@"%@/%@", QiNiuHost, resp[@"key"]];
