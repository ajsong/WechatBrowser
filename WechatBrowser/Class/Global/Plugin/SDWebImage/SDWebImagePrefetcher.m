    ...
    [self.manager downloadImageWithURL:self.prefetchURLs[index] options:self.options progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
        ...
        if (image) {
			...
			for (NSInteger i=0; i<self.imageArr.count; i++) {
				if ([self.imageArr[i] isKindOfClass:[NSString class]] && [self.imageArr[i] isEqualToString:imageURL.absoluteString]) {
					idx = i;
					break;
				}
			}
			if (idx != NSNotFound) {
				[self.imageArr replaceObjectAtIndex:idx withObject:image];
			}
        }
        else {
            if (self.progressBlock) {
        ...
        if (self.prefetchURLs.count > self.requestedCount) {
            ...
            if (self.completionBlock) {
                self.completionBlock(self.finishedCount, self.skippedCount, self.imageArr);
...
- (void)prefetchURLs:(NSArray *)urls progress:(SDWebImagePrefetcherProgressBlock)progressBlock completed:(SDWebImagePrefetcherCompletionBlock)completionBlock {
    ...
    if (urls.count == 0) {
        if (completionBlock) {
            completionBlock(0,0,nil);
        }
    } else {
		self.imageArr = [[NSMutableArray alloc]init];
		for (NSURL *url in urls) {
			[self.imageArr addObject:url.absoluteString];
		}
