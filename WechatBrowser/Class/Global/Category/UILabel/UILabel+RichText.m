...
- (void)clickWithStrings:(NSArray<NSString *> *)strings action:(void (^)(NSString *, NSRange, NSInteger))action {
	[self richTextRangesWithStrings:strings];
	if (self.clickBlock != action) {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
	...
	[self richTextFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
		...
		if ([weakself delegate] && [[weakself delegate] respondsToSelector:@selector(didClickString:range:index:)]) {
			[[weakself delegate] didClickString:string range:range index:index];
		}
